...
function initDahua(){
	WebVideoCtrl.insertPluginObject("divPlugin",$("#divPlugin").parent().width(),$("#divPlugin").parent().height());
	//初始化插件
...
function clickStartVoiceTalk(){
	...
	if(typeof info != "undefined"){
		WebVideoCtrl.startVoiceTalk(info.deviceID,{
				cbSuccess:function(){
					showOPInfo(szIP + " Enable AudioTalk Succeed");
				},
				cbFailed:function(){
					showOPInfo(szIP + " Enable AudioTalk Fail");
				}
			}
		)
	}
...
function clickOpenSound(){
	...
	if((playerID > 0) && WebVideoCtrl.controlAudio(playerID,true)){
		showOPInfo("Player: [" + winID.toString() + ","+ playerID.toString() + "]" + "Enable Sound Succeed");
	}else{
		showOPInfo("Player: [" + winID.toString() + ","+ playerID.toString() + "]" + "Enable Sound Fail");
	}
}
...
function clickCloseSound(){
	...
	if((playerID > 0) && WebVideoCtrl.controlAudio(playerID,false)){
		showOPInfo("Player: [" + winID.toString() + ","+ playerID.toString() + "]" + "Disable  Sound Succeed");
	}else{
		showOPInfo("Player: [" + winID.toString() + ","+ playerID.toString() + "]" + "Disable Sound Fail");
	}	
}
...
function clickRecordSearch(){
	// if(WebVideoCtrl.CLIENT_StartFileInfoInquiry()){
		// showOPInfo("Disable Enlarger Succeed");
	// }else{
		// showOPInfo("Disable Enlarger Fail");
	// }
